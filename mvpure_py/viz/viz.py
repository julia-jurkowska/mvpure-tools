...
def plot_RN_eigenvalues(
    ...
    lim_to_matrix_rank: bool = True,
    n_sources_threshold: float | None = None,
    ...
    **kwargs
    ...
    if lim_to_matrix_rank:
        cov_rank = np.linalg.matrix_rank(R)
        plt.scatter(x=range(1, cov_rank+1), y=eigval[:cov_rank], **kwargs)
    else:
        plt.scatter(x=range(1, len(eigval) + 1), y=eigval, **kwargs)
    if n_sources_threshold is not None:
        plt.hlines(
    ...
    plt.grid(alpha=0.2)
    if n_sources_threshold is not None or rank_threshold is not None:
        plt.legend()
    plt.show()
