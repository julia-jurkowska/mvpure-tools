...
def _check_rank(
        r: int | str | float | list,
        n_sources_to_localize: int):
    """
    Checking correctness of rank optimization ('r') parameter.
    """
    if not isinstance(r, (int, np.integer, str, float, np.floating, list)):
        raise ValueError(f"Optimization parameter 'r' (set to: {r}) should be of type: int, float, string or list. "
                         f"Got {type(r)} instead.")

    # if 'r' is a list, it should contain integers / floats
    # and all elements should be equal or less to n_sources_to_localize,
    if isinstance(r, list):
        if not all(
                isinstance(x, (int, np.integer, float, np.floating)) and
                n_sources_to_localize >= x >= 1
                for x in r
        ):
            raise ValueError("If `r` is a list, all elements should be int or float"
                             " and equal or less than number of sources to localize.")
